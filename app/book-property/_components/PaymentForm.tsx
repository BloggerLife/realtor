...
const PaymentForm = ({ handleSetPaymentSuccess }: PaymentFormProps) => {
  const { BookingData, resetBookingData } = useBookProperty();
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();
  const router = useRouter();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate a booking creation process without payment processing
      // You can replace this section with your custom booking logic
      // For example, making an API call to create a booking entry
      axios.post("/api/create-booking", { bookingData: BookingData });
